...
| Luna 模型 / 推理强度 | `gpt-5.6-luna` / `max`（设置页默认） | 直接调用可保留设置值；队列长文执行为避免 300 秒回合超时会限制为 `high`。                                                                |
...
为避免长篇全文在 Luna `max` 推理下持续生成而达到 300 秒回合上限，队列会显式给 Luna
回合传入 `reasoningEffort: high`。这不改变 Sol 的 `gpt-5.6-sol/high` 规划与验收，也不改变
设置页中直接调用端点的推理强度选择。
